...
[![Tested](https://img.shields.io/badge/tested-2.1.216-brightgreen.svg)](CHANGELOG_ZH.md)
...
追蹤於 [#62123](https://github.com/anthropics/claude-code/issues/62123)（62 留言，截至 2026-07-12 零官方回覆）及 [#67765](https://github.com/anthropics/claude-code/issues/67765)（根因分析）。
...
| **Hook**（`evap-shield.sh`） | PreToolUse hook，阻擋送給 **MCP tool** 的 `{}` 呼叫——這是 Claude Code 內建 validation 唯一沒守到的洞（見 [設計抉擇](#設計抉擇)）。判定依 per-session 歷史：只有「同一個 session 裡先送過非空參數」的 tool 收到 `{}` 才會被擋（這正是 VH1 中毒的特徵）；首發 `{}` 放行，因為合法的零參數 MCP tool 真實存在。兩種事件都會記錄，讓你知道 bug 有沒有在觸發。 | 是 |
...
| `bash check-update.sh` | 偵測到 Claude Code 更新時回報 VH1 patch 狀態（SessionStart hook 會跑這個） |
| `bash test-evap-shield.sh` | 跑 hook 測試套件（30 tests） |
| `bash test-patch-vh1.sh` | 跑 patch failure-path 測試（45 tests） |
| `bash test-install.sh` | 跑 installer 合併安全測試（26 tests） |
| `bash test-check-update.sh` | 跑更新偵測測試套件（21 tests） |
...
這會把 `evap-shield.sh`（PreToolUse）和 `check-update.sh`（SessionStart）複製到 `~/.claude/hooks/`，並把兩個都註冊到 `settings.json`。安裝前會先建立帶時間戳的 settings 備份；合併是冪等的，重跑只補缺的、不會重複加。
...
`install.sh` 還會註冊一個 **SessionStart hook**（`check-update.sh`），每次啟動時對 binary 做指紋比對。最常見的情況——沒變化——是靜默且近乎瞬間的（只 stat、不雜湊、不掃描）。只有當 Claude Code 真的更新了（更新會覆蓋 patch），它才會出聲：

- **vulnerable**——更新把 patch 洗掉了，它會提示你重跑 `patch-vh1.sh`。
- **unknown**——VH1 pattern 不見了，parser 可能被官方重構（也許修了），所以它請你手動確認。
- **patched / 無變化**——保持安靜。
...
evap-shield/
  ...
  check-update.sh       # SessionStart hook——更新覆蓋掉 patch 時回報
  install.sh            # 兩個 hook 的一鍵安裝
  patch-vh1.sh          # Binary patch 自動化（定位→備份→patch→驗證）
  test-evap-shield.sh   # Hook 測試套件（30 tests）
  test-patch-vh1.sh     # patch failure-path 測試（45 tests）
  test-install.sh       # installer 合併安全測試（26 tests）
  test-check-update.sh  # 更新偵測測試（21 tests）
...
Issue 有 57 留言，零官方回覆。唯一乾淨的出路是降回 Opus 4.6。
...
**Session 歷史閘門，不是無差別擋 `{}`。** Hook 讀不到 MCP schema，而合法的零參數 MCP tool 真實存在——無差別的「所有 MCP `{}` 都擋」會把它們的正常呼叫誤判成 bug（2026-06-30 真實環境實測：一次合法的零參數 context 呼叫被擋下，模型還被指示對一個健康的 session 執行 `/clear`）。所以 hook 改抓 bug 的實際特徵：VH1 中毒是 per-tool 且黏著的——tool 先送出真參數，然後塌縮成 `{}` 並一路空到底。Hook 以 session 為單位記錄哪些 tool 送過非空參數；只有帶著這個歷史的 tool 收到 `{}` 才會被擋。首發 `{}` 放行（記錄為 `allowed`），代價是第一次蒸發的呼叫會漏過去——通常只是無害的 server 端驗證錯誤——換來 false positive 近乎歸零。中毒的 tool 會一直空下去，所以第二次呼叫起照樣攔得住。
...
- **Hook 會放行首發 `{}`。** Session 歷史閘門只擋「同一個 session 裡已經送過非空參數」的 tool 的 `{}`。某個 tool 在 session 裡的*第一次*呼叫就蒸發的話，會被放行到 MCP server（通常得到一個 server 端驗證錯誤，並記錄為 `allowed`）——這是不誤擋合法零參數 tool 的代價：沒有歷史，`{}` 跟 bug 無從區分。參數全部 optional 的 tool 在送過非空呼叫之後，刻意送 `{}` 仍可能被擋；block 訊息裡有教模型怎麼表明那是刻意的。
...
- Patch 不會在 `claude update` 後存活。每次更新後重跑 `patch-vh1.sh`——SessionStart hook（`check-update.sh`）會偵測到更新並提醒你，但重 patch 這個動作本身刻意保持手動。
- Hook 無法阻止 model 在 hook 觸發前的 retry loop。error message 寫成 terminal instruction 要求 model 停手，但這依賴 model 是否遵從。
- **Model-side 的故障形態不在守備範圍——patch、hook、任何 client 端工具都一樣。** antml-prefix-dropped 家族（tool call 洩漏成 `<invoke>` XML 純文字、前面帶一個 c- 開頭的 stray token——`câ`/`call`/`court`/`count`）與 confabulation（捏造工具結果、在 thinking 裡腦內執行工具、假的「偵測到 prompt injection」／「環境損壞」警報）源自模型生成層，位於任何 client 可修之處的上游。這個 repo 對它們能提供的是分診而非防禦：辨認你中的是哪型指紋（空 `{}` 參數 vs 洩漏的 XML vs 與 transcript 對不上的聲稱），決定該 patch、retry、重開 session、還是重驗模型聲稱做過的工作。見 [#62123 的 2026-07-03 取證留言](https://github.com/anthropics/claude-code/issues/62123#issuecomment-4878159880)，以及 [investigation 報告 §8](docs/vh1-investigation.md#8-honest-scope-one-corner-of-the-cluster) 的 2026-07-12 量化調查——model-side 那一側正在加速。
