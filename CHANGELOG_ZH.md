...
- tested badge 更新到 **2.1.216**。Claude Code 從 2.1.215 → 2.1.216，版號連續，兩點 diff 不需要跨過任何沒裝的版本（`~/.local/share/claude/versions/` 現在有 214、215、216；2.1.203、2.1.209、2.1.213 仍是本機從未裝過的三個版本）。結構錨點掃原廠 binary 仍只找到 1 個 vulnerable site（bug 1／fix 0），字元級掃描迴圈簽名 `e[++t]` 仍出現 7 次。parser site 跟 2.1.215 逐字相同——±400 bytes window raw 就對上，連 normalize 都不必——仍是 `,!l)r.push({type:"string",value:a})`，flag（`l`）、接收端（`r`）、累加變數（`a`）都沒變——這下連續**十二版**（2.1.202、2.1.204、2.1.205、2.1.206、2.1.207、2.1.208、2.1.210、2.1.211、2.1.212、2.1.214、2.1.215、2.1.216）都帶同一組區域名稱。build 溯源：原廠 binary 長了 2,101,248 bytes（247,124,336 → 249,225,584），parser site 漂移 +1,817,780 bytes（217,546,581 → 219,364,361）；Bun banner 仍是 `Bun v1.4.0`，沒有 runtime 升級。Anthropic 自己的 2.1.216 changelog（40 條，直接抓下來 `grep -c` 機械計數——繼 2.1.215 的單條之後回到大版面；npm 顯示 2026-07-20T20:19Z 發布）沒有一條碰 tool-call parsing、JSON tokenization 或 streaming parser。聽起來最沾邊的幾條全在別的層：「Fixed Bash command parsing of non-ASCII characters to match real shell word boundaries」是 Bash 權限引擎的 shell 斷詞、不是 JSON；「Fixed a slowdown in long sessions where message normalization cost grew quadratically」是訊息簿記的效能修復；「Fixed cloud sessions dropping the in-flight message when the session's container restarts mid-turn」是 cloud session 續跑；這版的大宗是 worktree／background session 修復與權限硬化（PowerShell 隱形 Unicode 字元、Windows 網路路徑、帶重導向的複合陳述式）。strings diff（新增 5,089／移除 3,899）是近幾筆帳裡最乾淨的一次：新增集裡 charCode／codePoint／tokenizer 味的行**零條**（繼 2.1.206／2.1.207 的零條後再現；2.1.214 是 142 條、2.1.215 是 130 條 re-segmentation 噪音），15 條泛用 parse 命中落在 plugin eval、proxy response、Storybook adapter、cron prompt 解析、MCP config 載入——沒有一條是 VH1 site；主題計數（agent 87、worktree 36、permission 24、symlink 12、daemon 11、sandbox 10、oauth 10、chart palette 10）全對上 changelog 那 40 條。version-agnostic patcher 零腳本改動重套（`!l`→`!0`，1 byte；原廠 `d01b49210d72…` → patched `c7d83501e003…`，2026-07-21T02:35:36Z 完成），並在磁碟（bug 0／fix 1）、有效 ad-hoc 簽章、本 session 的 mmap inode 三處驗證（`lsof` 查本 session 自己的 process，PID 21973，執行檔解析到 `~/.local/share/claude/versions/2.1.216`，inode 60175821，patch 完成後 40 秒啟動；全機另一個活著的 claude process，PID 21681，持有同一個 patched inode）——patched dogfooding。（磁碟層註腳：re-sign 縮的還是那 1,184 bytes——原廠 CodeDirectory 15,181+7 個 hash slot、486,149 bytes，外加 9,047 bytes Developer ID 憑證鏈，換成 ad-hoc 的 15,181+2——精簡原廠版面就此**七連**：2.1.208、210、211、212、214、215、216。）這是 2.1.181 以來官方第 **26** 個有效改版（繼 2.1.183、185、186、187、190、191、193、195、196、197、198、199、200、201、202、204、205、206、207、208、210、211、212、214、215 後）仍未修 VH1。
...
- tested badge 更新到 **2.1.214**。Claude Code 從 2.1.212 → 2.1.214——跳過 2.1.213：該版本機從未裝過，而且不同於 2.1.209（至少還有 1 條 changelog），2.1.213 在官方 changelog **連段落都沒有**——npm 顯示 2.1.213 發布於 2026-07-17T22:26Z，107 分鐘後（2026-07-18T00:13Z）就被 2.1.214 取代，所以沒有東西需要跨越，兩點 diff 直接跑 212 ↔ 214（`~/.local/share/claude/versions/` 現在有 211、212、214；2.1.203、2.1.209、2.1.213 是本機從未裝過的三個版本）。結構錨點掃原廠 binary 仍只找到 1 個 vulnerable site（bug 1／fix 0），字元級掃描迴圈簽名 `e[++t]` 仍出現 7 次。parser site 跟 2.1.212 逐字相同——±260 bytes window raw 就對上，連 normalize 都不必——仍是 `,!l)r.push({type:"string",value:a})`，flag（`l`）、接收端（`r`）、累加變數（`a`）都沒變——這下連續**十版**（2.1.202、2.1.204、2.1.205、2.1.206、2.1.207、2.1.208、2.1.210、2.1.211、2.1.212、2.1.214）都帶同一組區域名稱。build 溯源：原廠 binary 長了 2,560,992 bytes（244,530,512 → 247,091,504），parser site 漂移 +2,377,380 bytes（215,144,209 → 217,521,589）；Bun banner 仍是 `Bun v1.4.0`，沒有 runtime 升級。Anthropic 自己的 2.1.214 changelog（47 條，機械 `grep -c` 直接抓官方檔案）沒有一條碰 tool-call parsing、JSON tokenization 或 streaming parser——*聽起來*最近的條目全落在別層：「修復 stream-json 輸出在 exit 時對慢速讀取的 SDK／管線消費者被截斷」（exit drain 改為隨排隊 bytes 縮放）是 CLI stream-json print-mode stdout，跟 2.1.208／210／211 同一層；「長時間工具呼叫新增週期性進度心跳」是渲染層 UI；這版的主打群——permission 檢查加固（單段 `dir/**` 規則、fd 重導形式、超過 10,000 字元的命令、`[[ ]]` 裡的 zsh 下標、PowerShell 5.1 繞過）——是 permission 引擎；EndConversation 工具是 model 面。有一條值得記在本 repo 的操作面備忘（而非 parser 帳上）：「修復 hook 的 stdout JSON 未通過 schema 驗證時 exit code 2 沒有照文件 block」碰到的是我們 PreToolUse 防護跑的 hook 引擎——但 `evap-shield.sh` 的 block 路徑只印到 **stderr**、以 exit 2 收尾、stdout 上什麼都沒有，從來不在壞掉的那條路徑上，行為不變。strings diff（新增 10,892／移除 11,116——判決帳裡首次移除多於新增，binary 卻長了 2.4 MB；`strings` 數的是不重複行數，不是 bytes）新增集裡有 142 條 charCode／codePoint／tokenizer 味的行，但全是熟面孔的 re-segmentation 收成：同一批既有 library 段落在本次 diff 和 2.1.212 的 diff 都出現（一段 ajv schema-codegen 模板——靠它的 `H_` → `w_` 改名可以跨兩對 diff 認出是同段——一個 PEM 解析器、PowerShell launcher 字串、highlight.js 文法定義），外加一條 named-pipe 的 `name.charCodeAt(0) === 0` 檢查、跟 2.1.208 排除過的同款；判決錨在結構錨點對 parser site window 的逐 byte 比對，不靠 strings diff。version-agnostic patcher 零腳本改動重套（`!l`→`!0`，1 byte；原廠 `59796dd18e9d…` → patched `69d048ad2b16…`，2026-07-18T04:15:44Z 完成），並在磁碟（bug 0／fix 1）、有效 ad-hoc 簽章、本 session 的 mmap inode 三處驗證（`lsof` 查本 session 自己的 process，PID 1800，執行檔解析到 `~/.local/share/claude/versions/2.1.214`，inode 59781012，patch 完成後 30 秒啟動；全機另一個活著的 claude process，PID 3725，持有同一個 patched inode）——patched dogfooding。一條磁碟層註腳，兼修正先前的定調：re-sign 又只讓檔案縮了 1,184 bytes（原廠 CodeDirectory 15,051+7 個 hash slot、481,989 bytes，外加 9,046 bytes Developer ID 憑證鏈，換成 ad-hoc 的 15,051+2）——精簡原廠版面就此**五連**（2.1.208、210、211、212、214）；2.1.207 之後重版面再沒出現過，先前「兩種版面交替」的讀法不再成立：精簡版面現在就是常態。這是 2.1.181 以來官方第 **24** 個有效改版（繼 2.1.183、185、186、187、190、191、193、195、196、197、198、199、200、201、202、204、205、206、207、208、210、211、212 後）仍未修 VH1。

- 補記 **2.1.212** 的覆驗——2026-07-17 安裝並 patch，帳目今天補登。Claude Code 從 2.1.211 → 2.1.212，版號連續，兩點 diff 不需要跨過任何沒裝的版本。結構錨點掃原廠 binary 只找到 1 個 vulnerable site（bug 1／fix 0），字元級掃描迴圈簽名 `e[++t]` 出現 7 次。parser site 跟 2.1.211 逐字相同——±260 bytes window raw 就對上，連 normalize 都不必——仍是 `,!l)r.push({type:"string",value:a})`，flag（`l`）、接收端（`r`）、累加變數（`a`）都沒變，同名連續第九版。build 溯源：原廠 binary 長了 2,084,832 bytes（242,445,680 → 244,530,512），parser site 漂移 +984,913 bytes（214,159,296 → 215,144,209）；Bun banner 仍是 `Bun v1.4.0`。Anthropic 自己的 2.1.212 changelog（48 條，機械 `grep -c`）沒有一條碰 tool-call parsing、JSON tokenization 或 streaming parser：這版是 session 編排面——`/fork` 改為複製出 background session（原本 session 內的行為改名 `/subtask`）、每 session 的 WebSearch 與 subagent 上限、MCP 呼叫超過 2 分鐘自動轉背景——加上一條 plan-mode permission 修復（會改檔的 Bash 命令現在會提示）和一條 worktree symlink 修復，全都離 parser 好幾層。strings diff（新增 8,523／移除 7,458）新增集裡有 141 條 charCode／codePoint／tokenizer 味的行——抽樣全是熟面孔的 re-segmentation 收成（ajv schema-codegen 模板段、PEM 解析器、PowerShell launcher 字串、highlight.js 文法定義），沒有一條是 VH1 site；新增集的主題跟官方 changelog 自己的主題對得上（fork／subtask、subagent 上限、sandbox、plugin 面）。patcher 在安裝當天零腳本改動套用（`!l`→`!0`，1 byte；原廠 `09ecba2ab2df…` → patched `95f78cf6ed4f…`，檔案 mtime 把 patch 定在 ~2026-07-17T04:05Z；state 檔其後已被 2.1.214 的 patch 覆蓋）。誠實補登邊界：安裝當天沒有抓 `lsof`，所以本條**不主張** 2.1.212 的活體 inode dogfooding——隔天覆驗涵蓋磁碟上的 patched binary（bug 0／fix 1，結構錨點 2026-07-18 重跑）和仍然有效的 ad-hoc 簽章（CodeDirectory 14,895+2；原廠是精簡版面 14,895+7 外加 9,047 bytes Developer ID 憑證鏈，re-sign 縮的還是那 1,184 bytes），且隔天 `lsof` 顯示已無任何 process 持有 2.1.212——它服完自己那一天的役，被 2.1.214 更新退役。這是 2.1.181 以來官方第 **23** 個有效改版仍未修 VH1。
...
- tested badge 更新到 **2.1.208**。Claude Code 從 2.1.207 → 2.1.208——版號再度連續，兩點 diff 不需要跨過任何沒裝的版本，但連續四天每日一更的節奏在週末停了（2.1.207 裝於週五 2026-07-11、2.1.208 裝於週一 2026-07-14，官方 changelog 兩版之間也沒有其他 build）；2.1.203 仍是唯一從未裝過的版本（`~/.local/share/claude/versions/` 現在有 206、207、208）。結構錨點掃原廠 binary 仍只找到 1 個 vulnerable site（bug 1／fix 0），字元級掃描迴圈簽名 `e[++t]` 仍出現 7 次。parser site 跟 2.1.207 逐字相同——±260 bytes window raw 就對上，連 normalize 都不必——仍是 `,!l)r.push({type:"string",value:a})`，flag（`l`）、接收端（`r`）、累加變數（`a`）都沒變——這下連續六版（2.1.202、2.1.204、2.1.205、2.1.206、2.1.207、2.1.208）都帶同一組區域名稱。build 溯源：原廠 binary 縮了 991,200 bytes（241,237,136 → 240,245,936）——判決帳裡繼 2.1.204 後第二次縮小，而且這次 changelog 沒有對應說法（該版一堆 memory／磁碟縮減條目講的都是 runtime RAM 和 transcript 磁碟用量，不是 binary 大小；不到 1 MB 落在正常版間波動內）——parser site 漂移 +252,838 bytes（211,935,208 → 212,188,046）；Bun banner 仍是 `Bun v1.4.0`，沒有 runtime 升級。Anthropic 自己的 2.1.208 changelog（45 條，判決帳裡最長的一份）沒有一條碰 tool-call parsing、JSON tokenization 或 streaming parser——*聽起來*沾邊的條目全落在 CLI 的 stream-json I/O 協定（給 SDK host 和 `claude -p` 管線用的 stdin/stdout JSON-lines 框架），跟 VH1 所在的 API 回應 tokenizer 是不同層：「piping 大量回應時 truncated stream-json/JSON 輸出與缺 result 訊息」是 print-mode stdout、「stream-json 輸入遇到空白 CRLF 或純空白行殺掉 session」是 SDK stdin 框架、「headless stream-json session 因非字串 `set_model` payload 永久卡住」是 SDK control 協定、Bedrock「Truncated event message received」修復是 gateway 錯誤重新命名。有一條值得記在本 repo 的操作面備忘（而非 parser 帳上）：「background session 在更新替換掉運行中 `claude agents` process 的 binary 後 attach 永久失敗」是官方在修 macOS patch 筆記記載的同一種 binary 替換 inode 解耦的症狀——更新管線，不是 parsing。strings diff（新增 6,132／移除 4,329）中斷了新增集 tokenizer 味連續兩版零條的紀錄，但只差一點：5 條命中，逐條排除——named-pipe 的 `charCodeAt(0)` 檢查、Bun 內部 `@String.fromCharCode` trace-event 發射、一批 TOML 風格「Unterminated basic string」錯誤字串表、一條 Rust 風格 `input[pos.cast()]` assertion、一條 shell 層「unterminated double quote」——沒有一條是字元級 JSON string tokenizer；26 條泛用 `parse` 命中同樣全落別處（session-store 匯入器跳過 unparseable 行、對磁碟檔案的 `JSON.parse`、lazy `parseUrl`、proxy-response 與 ANSI `parseFloat` 程式碼、ripgrep 自己的錯誤字串表、Bun bundler 的 `ParseTask`）。新增集跟官方 changelog 自己的主題對得上——background／daemon／agent 字串（96）、新的 `CLAUDE_CODE_PROCESS_WRAPPER` 企業 launcher 面（43）、memory-leak／LRU 工作（18）、transcript／checkpoint 修剪（9）——沒有一條伸進 parser。version-agnostic patcher 零腳本改動重套（`!l`→`!0`，1 byte——±260 window 裡恰好只有這 1 個 byte 不同；原廠 `051c7f28871b…` → patched `98652a9956ea…`），並在磁碟（bug 0／fix 1）、有效 ad-hoc 簽章、本 session 的 mmap inode 三處驗證（`lsof` 查本 session 自己的 process，PID 66678，執行檔解析到 `~/.local/share/claude/versions/2.1.208`，inode 59142150，patch 於 2026-07-14T02:32:06Z 完成後 88 秒啟動——patched dogfooding）。一條磁碟層註腳，兼修正 2.1.207 那條的定調：這次 re-sign 只讓檔案縮了 1,184 bytes，因為 2.1.208 原廠 CodeDirectory 回到精簡版面——14,634+7 個 hash slot（468,645 bytes）外加慣常的 9,046 bytes Developer ID 憑證鏈，被換成 14,634+2 slot 的 ad-hoc 版；2.1.202／206／207 是重版面、2.1.204／205／208 是精簡版面，兩種原廠簽章版面現在看是交替出現，而非精簡版面是特例——無論哪種都是簽章的機械性細節，沒有內容含義。這是 2.1.181 以來官方第 **20** 個有效改版（繼 2.1.183、185、186、187、190、191、193、195、196、197、198、199、200、201、202、204、205、206、207 後）仍未修 VH1。
